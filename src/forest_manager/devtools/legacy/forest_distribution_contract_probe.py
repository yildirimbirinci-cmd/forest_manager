...
def main() -> int:
    ...
    try:
        ...
        result = client.get_forest_distribution_contract()
        if not result.ok:
            print("Distribution contract query failed: " + result.error)
            return 3

        print("Forest Distribution Contract:")
        print(json.dumps(result.data, indent=2, ensure_ascii=True))
        print("Stage 3C distribution contract probe passed.")
        return 0
